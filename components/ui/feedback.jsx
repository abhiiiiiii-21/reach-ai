...
const Default = ({ label }) => {
    ...
    const getPosition = () => {
      if (isOpen && inputRef.current && menuRef.current) {
        ...
        if (buttonRect.bottom + menuHeight <= viewportHeight) {
          nextPosition = { top: buttonRect.height + 8 };
        } else if (buttonRect.top - menuHeight >= 0) {
          nextPosition = { bottom: buttonRect.height + 8 };
        } else {
          nextPosition = { top: buttonRect.height + 8 };
        }
  ...
  const handleEmojiToggle = (value) => {
    setSelectedEmoji((current) => (current === value ? null : value));
  };
  ...
  return (
    <div className="relative z-50">
      ...
      <Material
        type="menu"
        className={clsx(
          "absolute z-50 bg-background-100 w-[340px] duration-200 font-sans left-1/2 -translate-x-1/2",
        ...
        <form onSubmit={handleSubmit}>
          ...
          {submitted && !isSubmitting && (
            <div className="px-3 pb-3 text-sm text-emerald-600 text-right">
    ...
    <div
      ...
      <div
        ...
        <div className="flex justify-between">
          <span className="flex items-center gap-px cursor-pointer">
            <Button
              variant="unstyled"
              svgOnly
              shape="rounded"
